...
void TriangleShapeState::Update(float detlaTime)
{
...
void TriangleShapeState::CreateShape()
{
	...
	// triangle vertex input clockwise, so normal (visible face) faces camera
}

void DiamondShapeState::Update(float detlaTime)
{
	if (InputSystem::Get()->IsKeyPressed(KeyCode::UP))
	{
...
{
	mVertices.push_back({ {-0.5f,  0.0f, 0.0f}, Colors::LightBlue });
	mVertices.push_back({ {-0.25f,  0.25f, 0.0f}, Colors::LightBlue });
	mVertices.push_back({ {0.25f,  0.25f, 0.0f}, Colors::LightBlue });
	...
	mVertices.push_back({ {-0.5f,  0.0f, 0.0f}, Colors::LightBlue });
	mVertices.push_back({ {0.5f,  0.0f, 0.0f}, Colors::LightBlue });
	mVertices.push_back({ {0.0f,  -0.5f, 0.0f}, Colors::LightBlue });
}

void HeartShapeState::Update(float detlaTime)
{
	if (InputSystem::Get()->IsKeyPressed(KeyCode::UP))
	{
