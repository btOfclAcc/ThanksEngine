...
ThanksEngine::Graphics::RenderTarget::~RenderTarget()
{
	ASSERT(mRenderTargetView == nullptr && mDepthStencilView == nullptr, "RenderTarget: terminate must be called");
}

void ThanksEngine::Graphics::RenderTarget::Initialize(const std::filesystem::path& fileName)
{
	ASSERT(false, "RenderTarget: initialize not avalible in Render Target");
}

void ThanksEngine::Graphics::RenderTarget::Initialize(uint32_t width, uint32_t height, Format format)
{
...
void ThanksEngine::Graphics::RenderTarget::Terminate()
{
	Texture::Terminate();
...
void ThanksEngine::Graphics::RenderTarget::BeginRender(Color clearColor)
{
...
void ThanksEngine::Graphics::RenderTarget::EndRender()
{
	auto context = GraphicsSystem::Get()->GetContext();
