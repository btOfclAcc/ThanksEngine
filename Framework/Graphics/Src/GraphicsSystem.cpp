...
LRESULT CALLBACK GraphicsSystem::GraphicsSystemMessageHandler(HWND handler, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	return sWindowMessageHandler.ForwardMessage(handler, message, wParam, lParam);
}
...
void GraphicsSystem::Resize(uint32_t width, uint32_t height)
{
	...
	if (width != GetBackBufferWidth() || height != GetBackBufferHeight())
	{
		...
		ASSERT(SUCCEEDED(hr), "GraphicsSystem: failed to access swap chain");
